...
# the weak learner chosen is the decision tree classifier
...
ADB = AdaBoostClassifier(base_estimator = classifier)
plt.figure()
cv_scores = cross_val_score(ADB, arr_in, arr_out, cv=10)
...
plot_validation_curve(ADB, title2, xlabel, ylabel,X, y,param_name = 'n_estimators', ylim=None, 
                          cv=cv,n_jobs=4, param_range = np.arange(1,200,20))
