...
# Continuous Peaks Problem RHC
library(readr)


df.rhc = read_csv("./OptimizationResults/ContinuousPeaksRHC.csv")
...
p1_a = ggplot(df.rhc.median,aes(iteration,time)) + geom_point() + geom_line()

# Continuous Peaks Problem SA

df.sa = read.csv("./OptimizationResults/ContinuousPeaksSA_IT100.0_CF0.95-1.csv")
...
p2_a = ggplot(df.sa.median,aes(iteration,time)) + geom_point() + geom_line()

# Continuous Peaks Problem GA

df.ga = read.csv("/Users/jacquelineroudes/Documents/GTL_courses/Machine_Learning/Homework2/workspace/project/OptimizationResults/ContinuousPeaksGA_POP100_MAT50_MUT10-1.csv")
...
df.ga.median =aggregate(df.ga, list(iteration = df.ga$iteration,
                                    ...
                                    toMate = df.ga$toMate,
                                    toMutate=df.ga$toMutate), FUN = median)

p3=ggplot(df.ga.median,aes(iteration,fitness)) + geom_point() + geom_line()
p3_a = ggplot(df.ga.median,aes(iteration,time)) + geom_point() + geom_line()

# Continuous Peaks Problem MIMIC
df.mimic = read.csv("./OptimizationResults/ContinuousPeaksMIMIC_SAM100_KEP50-1.csv")
...
p4_a = ggplot(df.mimic.median,aes(iteration,time)) + geom_point() + geom_line()
