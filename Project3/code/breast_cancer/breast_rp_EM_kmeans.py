...
plt.text(6, 0.51, 'Breast Cancer', style='italic',
        bbox={'facecolor':'yellow', 'alpha':0.5, 'pad':2})
plt.grid(True)
plt.savefig('/Users/jacquelineroudes/Documents/GTL_courses/Machine_Learning/Homework3/images/breast_cancer/rp_silhouette_EM_Kmeans.png')
plt.show()  
...
plt.legend(bbox_to_anchor=(0.76, 0.99), loc=2, borderaxespad=0.)
plt.text(6, 0.61, 'Breast Cancer', style='italic',
        bbox={'facecolor':'yellow', 'alpha':0.5, 'pad':2})
plt.grid(True)
plt.savefig('/Users/jacquelineroudes/Documents/GTL_courses/Machine_Learning/Homework3/images/breast_cancer/rp_V_measure_EM_Kmeans.png')
plt.show()
...
kmeans = KMeans(init='k-means++', n_clusters= n_features, n_init=50)
...
plt.scatter(centroids[:, 0], centroids[:, 1],
            ...
            color='w', zorder=10)
plt.title('K-means clustering on the digits dataset (RP-reduced data)\n'
...
reduced_data = SparseRandomProjection(n_components = 3).fit_transform(data)
kmeans = KMeans(init='k-means++', n_clusters= n_features, n_init=50)
...
plt.title('EM clustering on the digits dataset (RP-reduced data)\n'
          'Centroids are marked with white cross')
plt.xlim(x_min, x_max)
plt.ylim(y_min, y_max)
plt.xticks(())
