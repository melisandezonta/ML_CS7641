...
ica = decomposition.FastICA(n_components = 2, whiten=True)
...
plt.text(6, 0.7, 'Breast Cancer', style='italic',
        bbox={'facecolor':'yellow', 'alpha':0.5, 'pad':2})
plt.grid(True)
plt.savefig('/Users/jacquelineroudes/Documents/GTL_courses/Machine_Learning/Homework3/images/breast_cancer/ica_silhouette_EM_Kmeans.png')
plt.show()  
...
plt.legend(bbox_to_anchor=(0.76, 0.99), loc=2, borderaxespad=0.)
plt.text(6, 0.72, 'Breast Cancer', style='italic',
        bbox={'facecolor':'yellow', 'alpha':0.5, 'pad':2})
plt.grid(True)
plt.savefig('/Users/jacquelineroudes/Documents/GTL_courses/Machine_Learning/Homework3/images/breast_cancer/ica_V_measure_EM_Kmeans.png')
plt.show()
...
reduced_data = decomposition.FastICA(n_components = 2, whiten=True).fit_transform(data)
...
plt.title('EM clustering on the digits dataset (ICA-reduced data)')
plt.xlim(x_min, x_max)
plt.ylim(y_min, y_max)
plt.xticks(())
plt.yticks(())
